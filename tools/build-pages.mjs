...
function docs(lang) {
  ...
  <a href="#install">${t.install}</a><a href="#prereq">${t.prereq}</a><a href="#hosts">${t.hosts}</a><a href="#theming">${t.theming}</a><a href="#languages">${t.languages}</a><a href="#forms">${t.forms}</a><a href="#lightdom">${t.lightdom}</a><a href="#icons">${t.icons}</a><a href="#overlays">${t.overlays}</a><a href="#editor">${t.editor}</a>`;
  const basicsHtml = `
...
<section class="site-section" id="languages"><h2>${t.languages}</h2><p class="doc-desc">${t.languagesBody}</p>${code(`<html lang="de">  <!-- the elements speak German: Kopieren, Heute, ＋ Neue Seite … -->\n\n<script type="module">\n  import { setStrings } from '@jungherz-de/notionkit-elements';\n  setStrings({ newPage: '＋ New course' });       // every language\n  setStrings({ copy: 'Copier' }, 'fr');           // a language of its own\n</script>\n\n<!-- an attribute wins -->\n<nk-copy-field value="https://…" copy-label="${W.copyLink}"></nk-copy-field>`)}</section>
