...
{
  tag: 'nk-copy-field', group: 'forms', classes: ['nk-copy-field', 'cf-value', 'cf-btn', 'copied', 'mono', 'wrap', 'wide', 'icons'],
  title: t('Copy field', 'Kopierfeld'),
  desc: t('A value to take along – a link, an address, a key – as tall as an input, with Copy inside on the right: it writes the clipboard and says “Copied” in green for a moment; where the clipboard is not allowed, the value is shown and selected for ⌘C. <code>secret</code> masks it behind Show/Hide, Copy still copies the real value. <code>value</code> set as a property is not reflected, so a key never lands in the markup. Each button carries an icon beside its word: on a phone, and with <code>icons</code> anywhere, the icon stands in for the word – two overlapping squares, an eye, a check in green for the moment after – and a masked key keeps a dozen characters more. The word stays the button’s name, and while the icons show, its tooltip for <code>nk-tooltip</code>.',
          'Ein Wert zum Mitnehmen – ein Link, eine Adresse, ein Schlüssel –, so hoch wie ein Eingabefeld, mit Kopieren rechts darin: Es schreibt die Zwischenablage und sagt kurz „Kopiert“ in Grün; wo die Zwischenablage nicht erlaubt ist, wird der Wert gezeigt und für ⌘C markiert. <code>secret</code> verbirgt ihn hinter Zeigen/Verbergen, Kopieren kopiert trotzdem den echten Wert. Als Property gesetzt, wird <code>value</code> nicht gespiegelt, ein Schlüssel landet also nie im Markup. Jeder Button trägt neben seinem Wort ein Symbol: Auf dem Telefon und mit <code>icons</code> überall steht das Symbol für das Wort – zwei überlappende Quadrate, ein Auge, ein grüner Haken für den Moment danach –, und ein verdeckter Schlüssel behält ein Dutzend Zeichen mehr. Das Wort bleibt der Name des Buttons und, solange die Symbole stehen, sein Tooltip für <code>nk-tooltip</code>.'),
  mobile: t('Keeps to its column – inside a panel even below its 210px: the value is cut, never the actions. The actions are icons, with the words as their names and tooltips.', 'Bleibt in seiner Spalte – in einem Panel auch unter seinen 210px: Gekürzt wird der Wert, nie die Aktionen. Die Aktionen sind Symbole, mit den Wörtern als Namen und Tooltips.'),
  attrs: [str('value', 'string', 'The value.', 'Der Wert.'), bool('secret', 'Masked, with Show/Hide.', 'Verborgen, mit Zeigen/Verbergen.'), bool('icons', 'Icons instead of words at every width, not only on a phone.', 'Symbole statt Wörter in jeder Breite, nicht nur auf dem Telefon.'), bool('mono', 'Monospace – addresses, keys, code.', 'Monospace – Adressen, Schlüssel, Code.'), bool('wrap', 'A long value breaks instead of an ellipsis.', 'Ein langer Wert bricht um statt Auslassungspunkten.'), bool('wide', 'Fills the row.', 'Füllt die Zeile.'), str('copy-label', 'string', 'Button text.', 'Button-Text.', { default: 'Copy · Kopieren' }), str('copied-label', 'string', 'Text for the moment after.', 'Text für den Moment danach.', { default: 'Copied · Kopiert' }), str('show-label', 'string', 'Reveal (secret).', 'Zeigen (secret).', { default: 'Show · Zeigen' }), str('hide-label', 'string', 'Mask again.', 'Wieder verbergen.', { default: 'Hide · Verbergen' })],
  slots: [],
  ...
  props: ['value', 'secret', 'icons'], methods: ['copy()'],
...
<div style="max-width:340px;margin-top:10px"><nk-copy-field value="ntn_4f2a9c1e8b7d6a5f3e2c" secret mono icons copy-label="${W.copyBtn}" show-label="${W.show}"></nk-copy-field></div>`,
  classMarkup: W => `<div style="max-width:340px"><div class="nk-copy-field"><span class="cf-value">https://monahilft.notionkit.app</span><button class="cf-btn" aria-label="${W.copyBtn}"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M15 9V5a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v8a2 2 0 0 0 2 2h4"/><rect x="9" y="9" width="12" height="12" rx="2"/></svg><span>${W.copyBtn}</span></button></div></div>
<div style="max-width:340px;margin-top:10px"><div class="nk-copy-field mono"><span class="cf-value">••••••••••••••••••••••••</span><button class="cf-btn" aria-label="${W.show}"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7S2 12 2 12Z"/><circle cx="12" cy="12" r="3"/></svg><span>${W.show}</span></button><button class="cf-btn" aria-label="${W.copyBtn}"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M15 9V5a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v8a2 2 0 0 0 2 2h4"/><rect x="9" y="9" width="12" height="12" rx="2"/></svg><span>${W.copyBtn}</span></button></div></div>
...
},
{
  ...
  attrs: [
    ...
    str('today-label', 'string', 'Today button.', 'Heute-Button.', { default: 'Today · Heute' }),
    str('prev-label', 'string', 'Names ‹.', 'Benennt ‹.', { default: 'Previous month · Voriger Monat' }), str('next-label', 'string', 'Names ›.', 'Benennt ›.', { default: 'Next month · Nächster Monat' }),
    str('week-label', 'string', 'Head of the week column.', 'Kopf der Wochenspalte.', { default: 'W · KW' }),
    str('time-label', 'string', 'Names the time field.', 'Benennt das Zeitfeld.', { default: 'Time · Uhrzeit' }), str('clear-label', 'string', 'Clear button.', 'Löschen-Button.', { default: 'Clear · Leeren' }),
    ...formAttrs,
...
{
  tag: 'nk-progress', group: 'content', classes: ['nk-progress', 'nk-progress-label', 'wide', 'nk-progress-row'],
  title: t('Progress', 'Fortschritt'),
  desc: t('A 60px bar with an optional label. <code>value</code>/<code>max</code> set the fill; the bar carries <code>role="progressbar"</code>. <code>wide</code> fills its row, and in a flex column – a panel – it stays a 6px bar. With a <code>label</code> the bar and its label form one <code>.nk-progress-row</code>: the label sits beside the bar, on its middle, in a panel too, where the host – <code>display: contents</code> – would let them stand one under the other.', 'Ein 60px-Balken mit optionalem Label. <code>value</code>/<code>max</code> setzen die Füllung; der Balken trägt <code>role="progressbar"</code>. <code>wide</code> füllt seine Zeile und bleibt in einer Flex-Spalte – einem Panel – ein 6px-Balken. Mit <code>label</code> bilden Balken und Label eine <code>.nk-progress-row</code>: Das Label steht neben dem Balken, auf seiner Mitte, auch in einem Panel, wo der Host – <code>display: contents</code> – sie sonst untereinander stellen würde.'),
  mobile: t('Unchanged.', 'Unverändert.'),
...
<div class="nk-panel" style="margin-top:12px"><h3>${W.dbProgress}</h3><nk-progress value="40" label="40${W.percent}" wide></nk-progress></div>`,
  classMarkup: W => `<span class="nk-progress-row"><span class="nk-progress"><i style="width:72%"></i></span><span class="nk-progress-label">72%</span></span>
<div class="nk-panel" style="margin-top:12px"><h3>${W.dbProgress}</h3><span class="nk-progress-row"><span class="nk-progress wide"><i style="width:40%"></i></span><span class="nk-progress-label">40${W.percent}</span></span></div>`,
},
{
...
// ============================================================ WAVE 3 · PAGE
{
  ...
  desc: t('The thumb-reachable twin of the sidebar for phones and installed PWAs. Put it last inside <code>nk-app</code>: it is slotted into the main column below the scrolling page, so it never moves and no bottom padding is needed. Keeps the <code>nk-tab-bar-item</code> whose value is <code>value</code> active (listening to <code>nk-select</code>); a <code>drawer</code> item opens the sidebar instead. A value no item has – a page without a tab of its own, opened from the drawer – marks the <code>drawer</code> item, as iOS marks “More”; without one no item is active. Needs NotionKit CSS 1.2.0.',
          'Der daumenfreundliche Zwilling der Sidebar für Telefone und installierte PWAs. Als letztes Kind von <code>nk-app</code> landet sie in der Hauptspalte unter der scrollenden Seite – sie bewegt sich nie, ein unteres Padding ist nirgends nötig. Hält das <code>nk-tab-bar-item</code> aktiv, dessen Wert <code>value</code> ist (hört <code>nk-select</code>); ein <code>drawer</code>-Eintrag öffnet stattdessen die Sidebar. Ein Wert, den kein Eintrag hat – eine Seite ohne eigenen Tab, aus der Schublade geöffnet –, markiert den <code>drawer</code>-Eintrag, wie iOS „Mehr“ markiert; ohne ihn ist keiner aktiv. Braucht NotionKit CSS 1.2.0.'),
...
},
{
  ...
  mobile: t('Unchanged.', 'Unverändert.'),
  attrs: [str('badge', 'string', 'Badge text.', 'Badge-Text.', { default: '⟳ synced · ⟳ synchronisiert' })],
...
},
{
  ...
  slots: [],
  events: [{ name: 'nk-select', detail: '{ index, value, step }', desc: t('A step clicked (with <code>selectable</code>, or its link with a plain click or Enter); cancel it to stay.', 'Ein Schritt geklickt (mit <code>selectable</code>, oder sein Link mit einfachem Klick oder Enter); abbrechen, um zu bleiben.') }],
...
<script>{ document.getElementById('stepsWizard').steps = [{ label: '${W.stepProvider}', state: 'done', href: '#nk-steps' }, { label: '${W.stepKey}', desc: '${W.stepSkipped}', state: 'skipped' }, '${W.stepTest}']; }</script>`,
...
<div style="margin-top:16px"><ol class="nk-steps horizontal" aria-label="${W.stepsLabel}">
  <li class="nk-step done"><span class="st-mark">✓</span><a class="st-label" href="#nk-steps">${W.stepProvider}</a></li>
...
},
{
  ...
  attrs: [bool('open', 'Shown.', 'Sichtbar.'), str('label', 'string', 'The dialog’s name – the entry’s title.', 'Der Name des Dialogs – der Titel des Eintrags.'), str('close-label', 'string', 'Name of ».', 'Name von ».', { default: 'Close · Schließen' }),
    ...
    str('resize-label', 'string', 'Name of the edge.', 'Name des Randes.', { default: 'Resize · Breite ändern' }),
...
{
  tag: 'nk-toast', group: 'overlays', classes: ['nk-toast', 'show', 't-action', 't-close'], frame: 120, overlay: true,
  title: t('Toast', 'Toast'),
  desc: t('One inverted pill at the bottom centre, above every overlay; over a tab bar at the bottom of the screen it rises 12px above the bar. <code>toast.show("Saved")</code> shows it and hides it after <code>duration</code> ms; <code>open</code> is the state. With an action – <code>show("Moved to trash", { action: { label: "Undo", value: "undo" } })</code> or a <code>&lt;button slot="action"&gt;</code> – it is Notion’s toast with a button and an ×: it stays until one of them is used, unless <code>show()</code> names a duration, and waits while the pointer or the focus is on it. The action fires <code>nk-action</code> and, unless cancelled, closes it; Escape closes it while the focus is inside.', 'Eine invertierte Pille unten mittig, über jedem Overlay; über einer Tab-Leiste am unteren Bildschirmrand steigt sie 12px über die Leiste. <code>toast.show("Gespeichert")</code> zeigt sie und blendet nach <code>duration</code> ms aus; <code>open</code> ist der Zustand. Mit einer Aktion – <code>show("In den Papierkorb verschoben", { action: { label: "Rückgängig", value: "undo" } })</code> oder einem <code>&lt;button slot="action"&gt;</code> – ist sie Notions Toast mit Button und ×: Sie bleibt, bis einer davon benutzt wird, außer <code>show()</code> nennt eine Dauer, und wartet, solange Zeiger oder Fokus auf ihr sind. Die Aktion feuert <code>nk-action</code> und schließt sie, sofern nicht abgebrochen; Escape schließt sie, solange der Fokus in ihr ist.'),
  mobile: t('Unchanged.', 'Unverändert.'),
  attrs: [bool('open', 'Visible.', 'Sichtbar.'), str('duration', 'ms', 'Auto-hide delay (0 = stay); a toast with an action stays unless show() names one.', 'Ausblend-Verzögerung (0 = bleibt); ein Toast mit Aktion bleibt, außer show() nennt eine.', { default: '2200' }), str('icon', 'string', 'Leading glyph.', 'Zeichen vorn.', { default: '✓' })],
  slots: [{ name: '(default)', desc: t('Static content (when <code>show()</code> gets no message).', 'Statischer Inhalt (wenn <code>show()</code> keine Nachricht bekommt).') }, { name: 'action', desc: t('A button for the action; its <code>value</code> goes out with <code>nk-action</code>.', 'Ein Button für die Aktion; sein <code>value</code> geht mit <code>nk-action</code> hinaus.') }],
  events: [{ name: 'nk-toggle', detail: '{ open }', desc: t('Shown / hidden.', 'Gezeigt / verborgen.') }, { name: 'nk-action', detail: '{ action, value, label }', desc: t('The action used; cancel it to keep the toast.', 'Die Aktion benutzt; abbrechen, um den Toast zu behalten.') }],
  methods: ['show(message?, { duration, action: { label, value } })', 'close()'], props: ['open', 'message'],
  example: W => `<nk-toast open duration="0" icon="🗑️">${W.trashed}<button slot="action" value="undo">${W.undo}</button></nk-toast>`,
  classMarkup: W => `<div class="nk-toast show">🗑️ <span>${W.trashed}</span><button class="t-action" type="button">${W.undo}</button><button class="t-close" type="button" aria-label="${W.closeLabel}"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M18 6 6 18M6 6l12 12"/></svg></button></div>`,
},
{
...
{
  tag: 'nk-gallery-view', group: 'data', classes: ['nk-gallery', 'small', 'large', 'fit', 'nk-card', 'card-item', 'nk-cover', 'card-title', 'card-meta', 'nk-new-row'], wide: true, script: true,
  title: t('Gallery view', 'Galerieansicht'),
  desc: t('The fifth database view: the rows as cards with a picture on top, in a grid that fills the row – Notion’s gallery, for a course catalog or a reading list. <code>cover-key</code> names the row field with the picture’s URL (default: <code>cover</code>); a row without one shows the cover gradient, <code>no-cover</code> leaves the pictures out. The picture stands in 2:1, cropped to fill; <code>fit</code> shows it whole, for logos. <code>size</code> small, medium or large sets the card size – columns from 180, 260 or 340px. Cards show the title and the <code>meta-keys</code> (default: the select and date columns) and fire <code>nk-select</code> on a click, Enter or Space; <code>new-row</code> adds the add card. <code>href-key</code> names the row field with a card’s address: such a card is a link, <code>&lt;a class="nk-card" href&gt;</code> in a <code>.card-item</code> that carries the list item’s role – to open a course in a new tab or copy its address. A plain click, and Enter, fire the same <code>nk-select</code>; cancel it and the browser stays. A middle click or one with Cmd, Ctrl, Shift or Alt fires nothing and does what a link does.',
          'Die fünfte Datenbank-Ansicht: die Einträge als Karten mit einem Bild oben, in einem Raster, das die Zeile füllt – Notions Galerie, für einen Kurskatalog oder eine Leseliste. <code>cover-key</code> nennt das Feld mit der URL des Bilds (Standard: <code>cover</code>); ein Eintrag ohne Bild zeigt den Cover-Verlauf, <code>no-cover</code> lässt die Bilder weg. Das Bild steht in 2:1 und wird beschnitten; <code>fit</code> zeigt es ganz, für Logos. <code>size</code> small, medium oder large setzt die Kartengröße – Spalten ab 180, 260 oder 340px. Karten zeigen den Titel und die <code>meta-keys</code> (Standard: die Select- und Datumsspalten) und feuern <code>nk-select</code> bei Klick, Enter oder Leertaste; <code>new-row</code> ergänzt die Hinzufügen-Karte. <code>href-key</code> nennt das Feld mit der Adresse einer Karte: Solch eine Karte ist ein Link, <code>&lt;a class="nk-card" href&gt;</code> in einem <code>.card-item</code>, das die Rolle des Listeneintrags trägt – um einen Kurs in einem neuen Tab zu öffnen oder seine Adresse zu kopieren. Ein einfacher Klick und Enter feuern dasselbe <code>nk-select</code>; bricht man es ab, bleibt der Browser. Ein Mittelklick oder einer mit Cmd, Strg, Umschalt oder Alt feuert nichts und tut, was ein Link tut.'),
...
},
{
  ...
  attrs: [
    ...
    str('filter-label', 'string', 'Text of the Filter tool.', 'Text des Filter-Werkzeugs.', { default: 'Filter' }), str('sort-label', 'string', 'Text of the Sort tool.', 'Text des Sortier-Werkzeugs.', { default: 'Sort · Sortieren' }),
    str('remove-label', 'string', 'The ×’s name, followed by the pill’s text.', 'Name des ×, gefolgt vom Text der Pill.', { default: 'Remove filter · Filter entfernen' }),
...
},
{
  ...
  mobile: t('Unchanged.', 'Unverändert.'),
  attrs: [str('placeholder', 'string', 'Input placeholder.', 'Platzhalter.'), str('send-label', 'string', 'Button text.', 'Button-Text.', { default: 'Send · Senden' }), bool('no-input', 'Read-only thread.', 'Nur lesen.'), bool('disabled', 'Input disabled.', 'Eingabe deaktiviert.')],
